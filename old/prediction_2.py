...
# Aktualisiere DataFrame und Features
df = df.iloc[valid_indices].reset_index(drop=True)
tab_features = tab_features[valid_indices]
...
X_train_img, X_test_img, X_train_tab, X_test_tab, y_train, y_test = train_test_split(
    images, tab_features_list, prices, test_size=0.2, random_state=42
)
...
for epoch in range(50):
    ...
    for (images, tab_data), prices in train_loader:
        images, tab_data, prices = images.to(device), tab_data.to(device), prices.to(device)
